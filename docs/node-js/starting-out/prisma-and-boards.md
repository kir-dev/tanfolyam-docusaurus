...
- **Kapcsolat (Relation):** Egy táblához (`Boards`) több hibajegy (`Ticket`) tartozhat. A `Ticket` modellben a `boardsId` fogja tárolni a kapcsolódó tábla azonosítóját (Foreign Key), amit a `@relation` annotáció köt össze.
...
_(`mo` = Module, `s` = Service — a CLI rövidítések ugyanúgy működnek, mint a `res` (Resource) a 2. fejezetben.)_
...
:::info
Ha elakadtál, akkor a [chapter-3](https://github.com/kir-dev/ticketing-api-2026/tree/chapter-3) branch-en megtalálod az eddigi kódot, amit összehasonlíthatsz a sajátoddal, vagy checkoutolhatod, hogy onnan folytasd.
:::
