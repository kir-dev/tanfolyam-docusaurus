...
Mielőtt belekezdenénk a Git használatába, töltsük le a git-et a gépünkre (amennyiben még nincs telepítve) a [git hivatalos oldaláról](https://git-scm.com/install/). Letöltés után győződjünk meg róla, hogy a parancssorban elérhető-e a `git` parancs a `git --version` paranccsal. Ha megjelenik a git verziószáma, akkor minden rendben van és készen állunk a következő lépésekre.

Hogyan is használjuk a Git-et? Milyen lépéseket kell végrehajtanunk ahhoz, hogy el tudjuk kezdeni a munkát? Ezt a [webes alapok](intro.md) előadáson elkészült demo segítségével fogjuk bemutatni. _[GitHub repository link](https://github.com/kir-dev/Git-Demo)_
...
Tovább gyakorlás érdekében ajánljuk a [Learn Git Branching](https://learngitbranching.js.org/) interaktív weboldalat, ahol vizuálisan is megérthetjük a Git működését és különböző parancsokat gyakorolhatunk.
